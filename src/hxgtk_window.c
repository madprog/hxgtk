...
value hxgtk_window_constructor(value title, value width, value height, value onClose ) {
...
value hxgtk_window_destroy(value win) {
	gtk_widget_destroy(val_data(win));
	return val_null;
}


value hxgtk_window_get_title(value win) {
	return alloc_string(gtk_window_get_title(val_data(win)));
}
value hxgtk_window_set_title(value win, value t) {
	gtk_window_set_title(val_data(win), val_string(t));
	return t;
}

value hxgtk_window_get_size(value win) {
...
value hxgtk_window_get_default_size(value win) {
	return _get_default_size(val_data(win));
}
value hxgtk_window_set_default_size(value win, value w,value h) {
	GtkWindow *_win = val_data(win);
	gtk_window_set_default_size(_win,val_int(w),val_int(h));
	return _get_default_size(_win);
}

value hxgtk_window_get_position(value w) {
	gint _w, _h;
	value o = alloc_object(NULL);
...
value hxgtk_window_set_position(value w, value p ) {
...
value hxgtk_window_get_destroy_with_parent(value win) {
	return alloc_bool(gtk_window_get_destroy_with_parent(val_data(win)));
}
value hxgtk_window_set_destroy_with_parent(value win, value v) {
	gtk_window_set_destroy_with_parent(val_data(win),val_bool(v));
	return v;
}

value hxgtk_window_is_active(value win) {
	return alloc_bool(gtk_window_is_active(val_data(win)));
}

value hxgtk_window_has_toplevel_focus(value win) {
	return alloc_bool(gtk_window_has_toplevel_focus(val_data(win)));
}

value hxgtk_window_present(value win) {
	gtk_window_present(val_data(win));
	return val_null;
}

value hxgtk_window_present_with_time(value win,value v) {
...
value hxgtk_window_stick(value win) {
	gtk_window_stick(val_data(win));
	return val_null;
}
value hxgtk_window_unstick(value win) {
...
value hxgtk_window_set_keep_above(value win, value v) {
	gtk_window_set_keep_above(val_data(win),val_bool(v));
	return val_null;
}
value hxgtk_window_set_keep_below(value win, value v) {
...
value hxgtk_window_get_icon_name(value win) {
	return alloc_string(gtk_window_get_icon_name(val_data(win)));
}
value hxgtk_window_set_icon_name(value win, value v) {
	gtk_window_set_icon_name(val_data(win), val_string(v) );
	return val_null;
}

value hxgtk_window_get_opacity(value win) {
	return alloc_float(gtk_window_get_opacity(val_data(win)));
}
value hxgtk_window_set_opacity(value win, value v) {
	gtk_window_set_opacity(val_data(win), val_float(v) );
	return val_null;
}

DEFINE_PRIM( hxgtk_window_constructor, 4);
